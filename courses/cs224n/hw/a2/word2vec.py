...
def naiveSoftmaxLossAndGradient(
    ...
    #logger.debug(f'outsideVectors {outsideVectors.shape}')
    #logger.debug(f'centerWordVec {centerWordVec.shape}')
    #logger.debug(f'prod {outsideVectors* centerWordVec}')
    ...
    # error = y-yhat, y =1 for outside vectors, 0 otherwise
    # (n_out)
    error = yHat.copy()
...
def negSamplingLossAndGradient(
    ...
    for k in range(len(indices)):
        ...
        # exclude the outside word at idx=0
        sample = indices[k]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    ### YOUR CODE HERE
    logger.debug(gradCenterVecs.shape)
    logger.debug(gradOutsideVecs.shape)
    logger.debug(f'window size {windowSize}')

    centerWordIdx = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[centerWordIdx]

    logger.debug(centerWordVec)
    ...
    for w in outsideWords:
        ...
        loss, _gradCenterVec, _gradOutsideVecs = word2vecLossAndGradient(
            centerWordVec,
            outsideWordIdx,
            outsideVectors,
            dataset
        )
        logger.debug(f'{gradCenterVecs[centerWordIdx].shape}')
        logger.debug(f'{_gradCenterVec.shape}')

        losses += loss
        gradCenterVecs[centerWordIdx] += np.ravel(_gradCenterVec)
        gradOutsideVecs += _gradOutsideVecs

        logger.debug(f'outside vector {outsideWordVec.shape}')
